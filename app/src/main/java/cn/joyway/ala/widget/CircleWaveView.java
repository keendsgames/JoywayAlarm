package cn.joyway.ala.widget;

import cn.joyway.ala.R;
import android.content.Context;
import android.graphics.Canvas;
import android.graphics.Paint;
import android.util.AttributeSet;
import android.view.View;

public class CircleWaveView extends View implements Runnable {
	float mWidth;
	float mHeight;

	float centerX;
	float centerY;
	float floatRadius;
	float maxRadius = 50;
	volatile boolean started = false;
	Paint mLinePaint;
	Paint mSolidPaint;
	// int waveColor = Color.argb(128, 0, 0, 0);
	int waveInterval = 50 * 2;
	boolean centerAlign = true;// ����
	float bottomMargin = 0;//
	boolean fillCircle = true;//

	Context context;

	public CircleWaveView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);

		this.context = context;
	}

	public CircleWaveView(Context context) {
		this(context, null, 0);
		this.context = context;
	}

	public CircleWaveView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		this.context = context;
		initView();
	}

	void initView() {
		mLinePaint = new Paint();
		mSolidPaint = new Paint();
	}

	void init() {
		mWidth = getWidth();
		mHeight = getHeight();

		mLinePaint.setAntiAlias(true);
		mLinePaint.setStrokeWidth(2.0F);
		mLinePaint.setStyle(Paint.Style.FILL_AND_STROKE);
		mLinePaint.setColor(context.getResources().getColor(R.color.color_ffffff));

		if (fillCircle) {
			mSolidPaint.setStyle(Paint.Style.FILL_AND_STROKE);
			mSolidPaint.setStrokeWidth(waveInterval);
			mSolidPaint.setColor(context.getResources().getColor(R.color.color_ffffff));
		}

		centerX = mWidth / 2.0F;
		if (centerAlign) {
			centerY = (mHeight / 2.0F);
		} else {
			centerY = mHeight - bottomMargin;
		}

		if (mWidth >= mHeight) {
			maxRadius = mHeight / 2.0F;
		} else {
			maxRadius = mWidth / 2.0F;
		}

		floatRadius = (maxRadius % waveInterval);

		start();
	}

	public void start() {
		if (!started) {
			started = true;
			new Thread(this).start();
		}
	}

	public void stop() {
		started = false;
	}

	protected void onDetachedFromWindow() {
		super.onDetachedFromWindow();
		stop();
	}

	protected void onDraw(Canvas canvas) {
		super.onDraw(canvas);
		if (maxRadius <= 0.0F) {
			return;
		}
		float radius = floatRadius % waveInterval;
		while (true) {
			int alpha = (int) (255.0F * (1.0F - radius / maxRadius));
			if (alpha <= 0) {
				break;
			}

			if (fillCircle) {
				mSolidPaint.setAlpha(alpha >> 2);
				canvas.drawCircle(centerX, centerY, radius - waveInterval / 2,
						mSolidPaint);
			}
			mLinePaint.setAlpha(alpha);
			canvas.drawCircle(centerX, centerY, radius, mLinePaint);
			radius += waveInterval;
		}
	}

	public void onWindowFocusChanged(boolean hasWindowFocus) {
		super.onWindowFocusChanged(hasWindowFocus);
		if (hasWindowFocus) {
			init();
		} else {
			stop();
		}
	}

	public void run() {
		while (started) {
			floatRadius = 5.0F + floatRadius;
			if (floatRadius > maxRadius) {
				floatRadius = (maxRadius % waveInterval);
			}
			postInvalidate();
			try {
				Thread.sleep(80L);
			} catch (InterruptedException localInterruptedException) {
				localInterruptedException.printStackTrace();
			}
		}
	}

	public void setMaxRadius(float maxRadius) {
		this.maxRadius = maxRadius;
	}

	// public void setWaveColor(int waveColor) {
	// this.waveColor = waveColor;
	// }

	public void setWaveInterval(int waveInterval) {
		this.waveInterval = waveInterval;
	}

	public void setCenterAlign(boolean centerAlign) {
		this.centerAlign = centerAlign;
	}
}
